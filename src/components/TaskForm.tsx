...
import { type FormErrors, createFormFieldsErrors } from '~/utils/form';
	...
	return (
		<div class="flex max-w-3xl flex-col gap-6">
			...
			<upsertTaskTrigger.Form class="flex max-w-xl flex-col gap-6">
				<Input error={upsertTaskErrors()['name']} name="name" value={props.task?.title ?? ''}>
					Name
				</Input>
				<TextEditor
				...
				<RadioGroup.Root value={props.task?.size ?? ''} name="size">
					...
					<Show when={upsertTaskErrors()['size']}>
						<p class="text-destructive pt-1 text-sm">{upsertTaskErrors()['size']}</p>
					</Show>
				</RadioGroup.Root>
				<RadioGroup.Root value={props.task?.size ?? ''} name="priority">
					...
					<Show when={upsertTaskErrors()['priority']}>
						<p class="text-destructive pt-1 text-sm">{upsertTaskErrors()['priority']}</p>
					</Show>
				</RadioGroup.Root>
